...
- **2025-11-16**: ~~Geocoder gem was configured for proximity search.~~ **UPDATED 2025-11-29**: Geocoding DISABLED due to Google Maps API costs. Using dummy default coordinates (WashU campus: 38.6488, -90.3108) via `before_validation :set_default_coordinates`. `near_location` scope uses raw SQL Haversine formula instead of geocoder's `near` method. TODO: Find free geocoding service.
...
- **2025-11-29**: ALWAYS mock external APIs in tests. Geocoder uses `:test` lookup in test env with stubbed responses (config/initializers/geocoder.rb). Never make real API calls in tests - they cost money, are slow, flaky, and non-deterministic. When adding ANY external service (geocoding, email, payment, etc.), immediately configure test stubs/mocks.
